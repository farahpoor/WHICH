<?php
	...
	echo "<table border='1'><tr><td>Row</td><td>File Name</td><td>Path</td><td>MD5 Hash</td><td>Status</td></tr>";
	foreach ($result_array as $path) {
		$filename=basename($path);
		$md5_hash=md5_file($path,false);		
		echo "<tr><td>" . $row_count . "</td><td>" . $filename . "</td><td>".$path ."</td><td>" . $md5_hash . "</td>";
		
		$sql = "SELECT * FROM file_md5 WHERE Source_File_Path='$path'";		
		...
		if ($result->num_rows > 0) 
		{
			...
			{
				if($row['MD5']==$md5_hash) {echo "<td><div style='color: #00cc66'>OK</div></td>"; $np++;}
				else {echo "<td><div style='color: red'>MD5 not matches!</div></td>"; $nm++; $problem_array[$problem_counter][0]=$path;$problem_array[$problem_counter][1]="MD5 Not Match";$problem_counter++;$flag=true;}
			}
		}
